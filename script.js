    ...
    if (e.type === "boss") {
      ...
      if (e.type === "reflector") {
        if (bullets.length > 0) {
          ...
          const moveSpeed = Math.min(2, distBullet / 20);
...
function updateDiamonds() {
  diamonds = diamonds.filter(d => {
    ...
    if (dist < player.size / 2 + d.size / 2) {
      score += 50; // diamond bonus
      return false;
    }
    return true;
  });
...
const waves = [
  ...
  { enemies: [{ type: "triangle", count: 2 }, { type: "normal", count: 3 }, { type: "reflector", count: 1 }] },
  { enemies: [{ type: "boss", count: 1 }] },
  { enemies: [{ type: "triangle", count: 2 }, { type: "normal", count: 3 }] },
  { enemies: [{ type: "boss", count: 1 }] },
  { enemies: [{ type: "normal", count: 5 }, { type: "triangle", count: 5 }, { type: "reflector", count: 5 }], diamond: true }, // wave 6 diamond
  { enemies: [{ type: "triangle", count: 5 }, { type: "normal", count: 5 }] },
  { enemies: [{ type: "mini-boss", count: 1 }, { type: "normal", count: 5 }] },
  { enemies: [{ type: "triangle", count: 5 }, { type: "mini-boss", count: 1 }, { type: "normal", count: 5 }], tunnel: true, diamond: true }, // wave 9 diamond
  { enemies: [{ type: "boss", count: 1 }, { type: "mini-boss", count: 3 }] }
];
...
function spawnWave(waveIndex) {
  ...
  if (waveData.enemies) {
    waveData.enemies.forEach(group => {
      ...
      else if (group.type === "reflector") for (let i = 0; i < group.count; i++) spawnReflector(canvas.width / 2 + i * 60 - 60, 200);
    });
  }

  if (waveData.diamond) {
    // Spawn 1 diamond in the middle of the canvas for simplicity
    spawnDiamond(canvas.width / 2, canvas.height / 3);
  }
}
