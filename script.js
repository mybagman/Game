...
function updateBoss(b){
  ...
  b.shootTimer++; if(b.shootTimer>150){ b.shootTimer=0;
    [{x:0,y:-1},{x:0,y:1},{x:-1,y:0},{x:1,y:0}].forEach(d=>lightning.push({x:b.x,y:b.y,dx:d.x*5,dy:d.y*5,size:6,damage:20}));
  }
}
...
function updateMiniBoss(b){
  ...
  b.shootTimer++; if(b.shootTimer>180){ b.shootTimer=0;
    [{x:0,y:-1},{x:0,y:1},{x:-1,y:0},{x:1,y:0}].forEach(d=>lightning.push({x:b.x,y:b.y,dx:d.x*5,dy:d.y*5,size:6,damage:10}));
  }
}
...
function updateEnemies(){
  enemies = enemies.filter(e=>{
    if(e.type==="boss"){ updateBoss(e); }
      ...
      if(e.type==="triangle"){ e.x+=(dx/dist)*e.speed; e.y+=(dy/dist)*e.speed; }
      else if(e.type==="shield" && e.shieldFor){ let tri=e.shieldFor; e.x=tri.x+e.offsetX; e.y=tri.y+e.offsetY; }
  ...
  for(let bi=bullets.length-1;bi>=0;bi--){ let b=bullets[bi];
    for(let ei=enemies.length-1;ei>=0;ei--){ let e=enemies[ei];
      if(Math.hypot(b.x-e.x,b.y-e.y)<e.size/2){ e.health-=10; bullets.splice(bi,1);
        if(e.health<=0){ createExplosion(e.x,e.y,(e.type==="triangle"?"cyan":e.type==="boss"?"yellow":e.type==="mini-boss"?"magenta":"red")); enemies.splice(ei,1); score+=(e.type==="boss"||e.type==="mini-boss"?100:10); } break;
      }
    }
  }
}
...
function drawEnemies(){ enemies.forEach(e=>{
  ...
  else if(e.type==="triangle"){ ctx.fillStyle="cyan"; ctx.beginPath(); ctx.moveTo(e.x,e.y-e.size/2); ctx.lineTo(e.x-e.size/2,e.y+e.size/2); ctx.lineTo(e.x+e.size/2,e.y+e.size/2); ctx.closePath(); ctx.fill(); return; }
  else if(e.type==="boss") ctx.fillStyle="yellow";
  else if(e.type==="mini-boss") ctx.fillStyle="magenta";
  else if(e.type==="shield") ctx.fillStyle="orange";
...
function nextWave(){
  if(enemies.length===0){
    ...
    switch(wave){
      ...
      case 2: spawnEnemies(2); spawnTriangleEnemies(1); break;
      case 3: spawnBoss(); break;
      case 4:
        let numTriangles=2;
        for(let i=0;i<numTriangles;i++){
          let triX=canvas.width/4 + i*(canvas.width/4), triY=100;
          let tri={x:triX,y:triY,size:30,speed:1.5,health:40,type:"triangle"};
          enemies.push(tri);
          for(let j=0;j<4;j++){
            let offsetX=(j%2===0?-1:1)*40, offsetY=(j<2?-1:1)*40;
            enemies.push({x:triX+offsetX,y:triY+offsetY,size:30,speed:2,health:30,type:"shield",shieldFor:tri,offsetX,offsetY});
          }
        }
        break;
      case 5: spawnEnemies(3); spawnTriangleEnemies(2); spawnMiniBoss(); break;
