...
<body>
  ...
  <script>
    ...
    let score = 0, wave = 0, minionsToAdd = [], tunnels = [], lastDir = { x: 1, y: 0 };
    ...
    function spawnRedSquares(count, fromBoss = false) {
    ...
    function spawnTriangles(count, fromBoss = false) {
      for (let i = 0; i < count; i++) {
        enemies.push({
    ...
    function spawnReflectors(count) {
      for (let i = 0; i < count; i++) {
        enemies.push({
          x: Math.random()*canvas.width, y: Math.random()*canvas.height,
          width: 40, height: 20, angle: 0, speed: 0.8, health: 200, type: "reflector"
        });
      }
    }
    function spawnBoss() {
    ...
    function spawnPowerUp(x, y, type) {
      powerUps.push({ x, y, type, size: 20, lifetime: 600 });
    }
    ...
    function updateGoldStar() {
      ...
      if (goldStar.collecting) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(goldStar.x, goldStar.y, goldStar.size/2+10+pulse, 0, Math.PI*2);
        ctx.stroke();
      ...
      if (nearest && minDist < 300) {
        ...
        goldStar.x += (dx/mag)*goldStar.speed;
        goldStar.y += (dy/mag)*goldStar.speed;
        if (minDist < 25) {
        ...
        if (dist > 100) {
          const mag = dist||1;
          goldStar.x += (dx/mag)*goldStar.speed*0.7;
          goldStar.y += (dy/mag)*goldStar.speed*0.7;
        }
      }
    ...
    function goldStarAttack() {
      if (goldStar.redPunchLevel > 0) {
        goldStar.punchCooldown++;
        if (goldStar.punchCooldown > 40) {
          goldStar.punchCooldown = 0;
          const range = goldStar.redPunchLevel === 1 ? 80 : 200;
          const punches = Math.min(goldStar.redPunchLevel, 5);
          let targets = enemies.filter(e => Math.hypot(e.x-goldStar.x, e.y-goldStar.y) < range).slice(0, punches);
          targets.forEach(e => {
            e.health -= 25;
      ...
      if (goldStar.blueCannonnLevel > 0) {
        ...
        if (goldStar.cannonCooldown > 50) {
          ...
          if (enemies.length > 0) {
            ...
            const dx = target.x-goldStar.x, dy = target.y-goldStar.y, mag = Math.hypot(dx,dy)||1;

            if (goldStar.blueCannonnLevel === 1) {
              bullets.push({ x: goldStar.x, y: goldStar.y, dx: (dx/mag)*8, dy: (dy/mag)*8, size: 8 });
            } else if (goldStar.blueCannonnLevel === 2) {
              bullets.push({ x: goldStar.x, y: goldStar.y-5, dx: (dx/mag)*8, dy: (dy/mag)*8, size: 8 });
              bullets.push({ x: goldStar.x, y: goldStar.y+5, dx: (dx/mag)*8, dy: (dy/mag)*8, size: 8 });
            } else if (goldStar.blueCannonnLevel === 3) {
              for (let i = -1; i <= 1; i++) {
                const angle = Math.atan2(dy,dx)+i*0.3;
                bullets.push({ x: goldStar.x, y: goldStar.y, dx: Math.cos(angle)*8, dy: Math.sin(angle)*8, size: 8 });
              }
              ...
              for (let i = 0; i < 5; i++) {
                bullets.push({ x: goldStar.x+(dx/mag)*i*20, y: goldStar.y+(dy/mag)*i*20, dx: (dx/mag)*12, dy: (dy/mag)*12, size: 10 });
              }
            }
          }
        }
    ...
    function drawGoldStar() {
      ...
      const barWidth = 50;
      ctx.fillStyle = "gray";
      ctx.fillRect(goldStar.x-barWidth/2, goldStar.y-goldStar.size, barWidth, 5);
      ctx.fillStyle = "gold";
      ctx.fillRect(goldStar.x-barWidth/2, goldStar.y-goldStar.size, barWidth*(goldStar.health/goldStar.maxHealth), 5);
    }

    function spawnDiamondEnemy(x = canvas.width/2, y = canvas.height/3) {
      diamonds.push({
        x, y, size: 40, health: 200, type: "diamond", attachments: [],
        canReflect: false, angle: Math.random()*Math.PI*2, shootTimer: 0, pulse: 0
      });
    }
    ...
    function attractEnemiesToDiamond(diamond, allEnemies) {
      for (let i = allEnemies.length-1; i >= 0; i--) {
        ...
        const dx = diamond.x-e.x, dy = diamond.y-e.y, dist = Math.hypot(dx,dy);
    ...
    function diamondDeath(d) {
      ...
      d.attachments.forEach(a => {
        enemies.push({
          ...
          size: a.size ? Math.max(12, a.size*0.6) : 15, speed: 2,
          health: Math.max(10, (a.health||20)*0.5),
          type: a.type==="reflector" ? "reflector" : (a.type==="triangle" ? "triangle" : "red-square"),
          shootTimer: 0, fromBoss: true
        });
      });
    }

    function spawnTunnel() {
      const wallHeight = canvas.height/3, wallWidth = 600;
      tunnels.push(
        { x: canvas.width, y: 0, width: wallWidth, height: wallHeight, speed: 2, active: true },
        { x: canvas.width, y: canvas.height-wallHeight, width: wallWidth, height: wallHeight, speed: 2, active: true }
      );
    }
    ...
    function updateBoss(boss) {
      ...
      if (boss.spawnTimer > 200) {
        ...
        minionsToAdd.push({
          ...
          size: 30, speed: 2, health: 30, type: "red-square", shootTimer: 0, fromBoss: true
        });
      }
    ...
    function updateMiniBoss(boss) {
      ...
      if (boss.spawnTimer > 300) {
        boss.spawnTimer = 0;
        minionsToAdd.push({ 
          x: boss.x+(Math.random()-0.5)*80, y: boss.y+(Math.random()-0.5)*80,
          size: 25, speed:2, health:30, type: "red-square", shootTimer:0, fromBoss: true
        });
      }
      boss.shootTimer = boss.shootTimer||0;
      boss.shootTimer++;
    ...
    function updateEnemies() {
      ...
      enemies = enemies.filter(e => {
        ...
        if (e.type === "reflector") {
          ...
          const distToPlayer = Math.hypot(e.x-player.x, e.y-player.y);
          if (distToPlayer < 30) {
            if (!player.invulnerable) player.health -= 15;
    ...
    function updateDiamond(d) {
      ...
      if (d.attachments.some(a=>a.spawnMini) && d.shootTimer % 200 === 0) {
        minionsToAdd.push({
          x: d.x+(Math.random()-0.5)*80, y: d.y+(Math.random()-0.5)*80,
          size: 25, speed: 2, health: 30, type: "red-square", shootTimer: 0, fromBoss: true
        });
      }
    ...
    function checkBulletCollisions() {
      for (let bi = bullets.length-1; bi >= 0; bi--) {
              ...
              if (e.health <= 0) {
                ...
                if (!e.fromBoss) {
                  if (e.type === "boss") score += 100;
                    ...
                    score += 10;
                    spawnPowerUp(e.x, e.y, "blue-cannon");
                  } else if (e.type === "red-square") {
                    score += 10;
                    spawnPowerUp(e.x, e.y, "red-punch");
                  }
                }
              }
              break;
        ...
        if (bi >= 0 && bi < bullets.length) {
          ...
          for (let di = diamonds.length-1; di >= 0; di--) {
            const d = diamonds[di];
            if (Math.hypot(b.x-d.x, b.y-d.y) < d.size/2) {
              d.health -= 10;
              bullets.splice(bi,1);
              break;
            }
    ...
    function drawUI() {
      ...
      ctx.fillText(`Wave: ${wave+1}`, 20, 90);
